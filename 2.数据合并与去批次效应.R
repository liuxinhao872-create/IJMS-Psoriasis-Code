...
# 1. 数据路径与文件夹自动新建
data_path <- "E:/1paper/合并数据"
out_dirname <- paste0("BatchEffect  _output_", format(Sys.time(), "%Y%m%d_%H%M%S"))
...
write.table(corrected_data, file = file.path(output_dir, "merged_after_batch_removal.txt"),
  sep = "\t", row.names = FALSE, quote = FALSE)
