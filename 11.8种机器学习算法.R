...
# 定义输入文件路径
inputFile="merged_after_batch_removal.txt"      # 基因表达数据文件
geneFile="gene.txt"      # 基因列表文件

# 设置工作目录
setwd("E:/1paper/9种机器学习算法模型来筛选疾病诊断特征基因")
...
explainer_nnet=explain(mod_nnet, label = "NNET",
                       ...
                       predict_function = p_fun,
                       verbose = FALSE)
mp_lasso=model_performance(explainer_nnet)
...
explainer_lasso=explain(mod_lasso, label = "LASSO",
                        ...
                        predict_function = p_fun,
                        verbose = FALSE)
mp_nnet=model_performance(explainer_lasso)
...
pred4=predict(mod_gbm, newdata=test, type="prob")
pred5=predict(mod_knn, newdata=test, type="prob")
pred6=predict(mod_nnet, newdata=test, type="prob")
pred7=predict(mod_lasso, newdata=test, type="prob")
pred8=predict(mod_dt, newdata=test, type="prob")
...
legend('bottomright',
       ...
       col=c("red","blue","green","yellow","orange","purple","black","blue"), lwd=2, bty = 'n')
dev.off()
